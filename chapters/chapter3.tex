...
% \begin{align*}
% 	a\coloneqq 1 \\
% 	\textit{x\_}t\coloneqq t\hiderel{\mapsto }a\cdot \left(1+\cos \left(t\right)\right)\cdot \cos \left(t\right) \\
% 	\textit{y\_}t\coloneqq t\hiderel{\mapsto }a\cdot \left(1+\cos \left(t\right)\right)\cdot \sin \left(t\right)
% \end{align*}
...
\begin{proof}
	...
	\rom{3}. either $\gamma_1 \bigcap \gamma_2 = \emptyset$, or $ \gamma_1 = \gamma_2 \forall \gamma_1, \gamma_2 \in \Gamma_{U,\Phi}$ \par
	\rom{4}. $\bigcup_{\gamma\in \Gamma_U} \gamma = U$ \par

	\rom{1} is true by Def. of $\gamma_s$. \rom{2} as well, since $\Phi(0,s) = s$, it contains at least the initial condition.

	For \rom{3}, take $\gamma_{s_1}, \gamma_{s_2} \in \Gamma_{U, \Phi}$, and assume $\exists s \in \gamma_{s_1} \bigcap \gamma_{s_2} \implies \exists t_1,t_2 \in T : s = \Phi(t_1,s_1) = \Phi(t_2, s_2)$. Solving for $s_1 \implies s_1 = \Phi(t_2-t_1, s_2)$ meaning $s_1$ is on the orbit made by $s_2$, so $\gamma_{s_1} \subseteq \gamma_{s_2}$, conversely we can show $\gamma_{s_2} \subseteq \gamma_{s_1} \implies \gamma_{s_1} = \gamma_{s_2}$. So $\gamma_{s_1}, \gamma_{s_2}$ are either disjoint or one and the same.

	From \rom{1} we know $\forall \gamma \subseteq U \implies \bigcup_{\gamma\in \Gamma_U} \subseteq U$. Take $\forall s \in U \implies \gamma_s \in \Gamma_U$, from \rom{2} $\implies U \subseteq \bigcup_{\gamma\in \Gamma_U}$, \rom{4} checked
\end{proof}
...
\vskip\bigskipamount
//TODO: faci tu dupa aici un plot in matlab / maple frumusel
...
Hurwitz \cite{hairer1993Nonstiff}:
...
He's shown that condition \rom{2} of \ref{routh_theorem} (hence $\text{ Re }(\lambda)< 0, \forall \lambda \in p^{-1}(0)$ with $a_0 > 0$ as well) $\iff \det (H_i) \forall i = \overline{1,n}$

So this is what's used to show the stability of the linear system (or linearization of a non-linear one), computing matrices of coefficients of the charracteristic polynomial, instead of having to compute Eig($J$) directly.
\end{theorem}

We'll use this later during the section about \textbf{bifurcations} as well.
