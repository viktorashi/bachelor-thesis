...
Althought, like a coin left standing on its side, even the slightest deviation will get it tumbling down. This can be observed in the system's phase portrait.
...
In general, of a system: \ref{fo_system_auton_eq}, written in the compact form
\[
\underline{\dot{y}}(t) = \underline{f}(\underline{y}(t)).
\]
...
$ \underline{f}(y_0) = 0 = \underline{\dot{y}}(t) $
...
$\qquad \qquad \forall \epsilon > 0, \exists \delta > 0 \text{ s.t. } $:
...
\begin{equation} \label{cond:stable2}
  \text{\rom{2}: } \norm{y_0 - y(t)} < \epsilon \quad \forall t >= 0. \tag{\rom{2}}
\end{equation}

\textbf{Unstable equilibriums}:
If either \eqref{cond:stable1} or \eqref{cond:stable2} don't hold

\textbf{Asymptotically stable equilibriums}:
