...
So for a nonlinear system $\dot{y} = \underline{f}(y)$ and its lin. approx. \ref{eq:lin_approx} $\dot{\zeta} = J(y_f)\zeta$ about fixed state $y_f$ we can only show if (in the context of the non-linear system) that fixed point is either:
...
$Re(\lambda) \leqslant  0 \forall \lambda \in Eig(J(y_f)) \text{ but } \exists \lambda : Re(\lambda) = 0, \text{ and } \exists \lambda : Re(\lambda) < 0 $ the results are inconclusive and can't be implied for the non-linear system. This criticial condition, if we're talking about th system's dependece in relation to some constant parameter, corresponds to a \textbf{local bifurcation} which shall be discussed in more detail later.

// TODO: HAI MA CA NU ESTI PROST invata si despre lyapuov mai bine sa poti sa vorbesti despre dansul dupa ce zici din astea mai de specifice de chimie ca sigur o sa ajute cu ceva na pana mea
