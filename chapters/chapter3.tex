...
$Re(\lambda) \leqslant  0 \forall \lambda \in Eig(J(y_f)) \text{ and, at least } \exists \lambda : Re(\lambda) = 0$ then the results are inconclusive and can't be implied for the non-linear system. This criticial condition, if we're talking about the system's dependece in relation to some constant parameter, corresponds to a \textbf{local bifurcation} which shall be discussed in more detail later.
